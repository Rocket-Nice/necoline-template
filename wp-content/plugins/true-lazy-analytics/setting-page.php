...
<div class="wrap">    
      <h1  style="display:inline;">True Lazy Analytics <small>v<?php echo TLAP_VERSION; ?></small></h1>  
   		<h2 class="nav-tab-wrapper">
			<a href="<?php echo admin_url( 'admin.php?page='.TLAP_SLUG ); ?>" class="nav-tab<?php if ( !isset( $_GET['action'] ) || isset( $_GET['action'] ) && 'second-tab' != $_GET['action']  && 'third-tab' != $_GET['action'] && 'fourth-tab' != $_GET['action']) echo ' nav-tab-active'; ?>"><span class="dashicons dashicons-admin-generic"></span><?php echo __('Main Settings', 'true-lazy-analytics'); ?></a>
			...
			<a href="<?php echo esc_url( add_query_arg( array( 'action' => 'third-tab' ), admin_url( 'admin.php?page='.TLAP_SLUG ) ) ); ?>" class="nav-tab<?php if ( $third_tab ) echo ' nav-tab-active'; ?>"><span class="dashicons dashicons-chart-bar"></span><?php echo __('Yandex Metrica', 'true-lazy-analytics'); ?></a>
			<a href="<?php echo esc_url( add_query_arg( array( 'action' => 'fourth-tab' ), admin_url( 'admin.php?page='.TLAP_SLUG ) ) ); ?>" class="nav-tab<?php if ( $fourth_tab ) echo ' nav-tab-active'; ?>"><span class="dashicons dashicons-performance"></span><?php echo __('Speed Up Your Website', 'true-lazy-analytics'); ?></a>
			</h2>
	<div id="poststuff">
...
function tlap_fill_exclude_pages(){
	...
	?>
<span class="tlap-field-premium-icon"><input size="60" type="text" name="tlap_add_analytics_option_main[tlap_excludepage]" value="<?php echo esc_attr( $val ) ?>" placeholder="<?php echo __('Еnter the Page IDs (separated by commas), for example: 345,1145,3778', 'true-lazy-analytics'); ?>" disabled="disabled" />	</span>
<div><?php echo esc_html__('Excluded pages - pages on which the code of analytics systems will not be displayed. For example, on pages with the <meta name="robots" content="noindex" /> tag', 'true-lazy-analytics'); ?></div>
	<?php
}
...
function tlap_fill_analytics_id(){
	...
	<div style="color:red;"><?php echo __('Attention! Before installing the analytics counter through our plugin, do not forget to remove the code of the analytics counter installed before.', 'true-lazy-analytics'); ?>
	</div>
	<?php
}
...
function tlap_fill_yametrika_id(){
	...
	<div style="color:red;"><?php echo __('Attention! Before installing the analytics counter through our plugin, do not forget to remove the code of the analytics counter installed before.', 'true-lazy-analytics'); ?></div>
	<?php
}
...
function tlap_fill_yametrika_cdn( ) {
    ...
    <span>
		...
		<input type="radio" name="tlap_add_analytics_option_metrica[tlap_yametrika_cdn]" value="1" <?php checked( $val, 1 ); ?>><?php echo __( 'direct', 'true-lazy-analytics' ); ?>
	</span>
<div><?php echo __('Attention! By default, the code is added via CDN. Alternative CDN allows you to correctly take into account visits from regions where access to Yandex resources is limited. Using this option may reduce the loading speed of the counter code.', 'true-lazy-analytics'); ?></div>
    <?php
}
/*
****************************************************************
...
<p style="text-indent: 10px;text-align: justify;"><?php echo __('This plugin is designed to speed up your sites and increase your PageSpeed test scores. You can speed up your site using delayed loading of analytics counter code.', 'true-lazy-analytics'); ?></p>
<p style="text-indent: 10px;text-align: justify;"><?php echo __('If you have a complex project created on the basis of page builders (such as Elementor) or you just want to carry out additional optimization of the site, you can turn for help to the experts by clicking on the "Order" button.', 'true-lazy-analytics'); ?>
</p>
</div>
<div>	
